...
#region Operations
        ...
        private static void blah(){
            ClearPrevData();
            foreach(RingNodes rn in PublicParameters.RingNodes){

                foreach(NeighborsTableEntry ringNeiEntry in rn.Node.NeighborsTable){

                    if(!(ringNeiEntry.NeiNode.RingNodesRule.isRingNode)){
                        ringNeiEntry.NeiNode.RingNeighborRule.NeighboringRingNodes.Add(rn.Node);
                        if (!(NeiNodesHolder.Contains(ringNeiEntry.NeiNode)))
                        {
                            NeiNodesHolder.Add(ringNeiEntry.NeiNode);
                        }
                    }
                }
        ...
        private static void GetNearestRingNodeFromMe(){
            ...
            foreach (Sensor sen in NeiNodesHolder)
            {
                ...
                foreach (NeighborsTableEntry neiEntry in sen.NeighborsTable)
                {
                    Sensor node = neiEntry.NeiNode;
                    ...
                    foreach (NeighborsTableEntry neiNeiEntry in node.NeighborsTable)
                    {
                        Sensor node2 = neiEntry.NeiNode;
